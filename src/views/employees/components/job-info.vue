<template>
  <div class="job-info">
    ...
    <el-form label-width="220px">
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="saveEmployessJobs">保存更新</el-button>
...
import { getEmployessJobInfo, saveEmployessJobInfo } from '@/api/employees'
export default {
  data() {
    return {
  ...
  created() {
    this.loadEmployessJob()
  },
  methods: {
    async loadEmployessJob() {
      this.formData = await getEmployessJobInfo(this.userId)
    },
    async saveEmployessJobs() {
      try {
        await saveEmployessJobInfo(this.formData)
