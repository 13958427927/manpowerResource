...
// 2,布局的时候隐藏input
// 3.点击上传实际点击的是input
// 4,利用input的change事件拿到上传的文件，然后再进行上传或者是读取数据的操作
// 5.出于上传文件之前可能需要对文件校验的场景提供beforeUpload字段 可以使用者自由定义校验规则，
// 6,利用XLSX插件读取x1s×文件里的数据，拿到数据，对数据进行整合
// 7,把整合的数据抛出去
// 8,这个组件给父组件提供了两个参数beforeUpload上传前的校验onSuccess上传成功拿到数据之后可以通过这个回调把数据给父组件
